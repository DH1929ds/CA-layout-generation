...
class CAL_6(ModelMixin, ConfigMixin):
    ...
    def forward(self, sample, noisy_sample, timesteps):
        ...
        if self.is_cond:
            mask_num = int(0.2 * sample['geometry'].size(0))
            mask_index = torch.randperm(sample['geometry'].size(0))[:mask_num]
            
            image[mask_index] = 0
            ratio[mask_index] = 0
            cat_input[mask_index] = 0
        ...
        tokens_emb = torch.cat([xy_emb, wh_emb, r_emb, z_emb, image_emb, elem_cat_emb, ratio_emb], dim=-1) #concat
