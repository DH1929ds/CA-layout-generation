...
class CAL_6(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(self, latent_dim=512, num_layers=16, num_heads=16, dropout_r=0., activation="gelu",
                 geometry_dim=256):
        super().__init__()
    ...
    def forward(self, sample, noisy_sample, timesteps):
        ...
        r_cos = torch.cos(sample["geometry"][:, :, 4] * 2 * torch.pi)
        r_sin = torch.sin(sample["geometry"][:, :, 4] * 2 * torch.pi)
        r_concatenated = torch.cat([r_cos.unsqueeze(-1), r_sin.unsqueeze(-1)], dim=-1)

        # Concatenated vector를 embedding 층에 전달
        r_emb = self.r_emb(r_concatenated)
        
        z = sample["geometry"][:, :, 5].unsqueeze(-1)
        z_emb = self.z_emb(z)
...
class CAL_4(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(self, latent_dim=512, num_layers=8, num_heads=8, dropout_r=0., activation="gelu",
                 geometry_dim=256):
        super().__init__()
        self.latent_dim = latent_dim
        self.dropout_r = dropout_r
    ...
    def forward(self, sample, noisy_sample, timesteps):
        ...
        #tokens_emb = torch.cat([image_emb, xy_emb, wh_emb, r_emb, z_emb], dim=-1) #concat
        tokens_emb = self.tokens_emb(tokens_emb)
        tokens_emb = rearrange(tokens_emb, 'b c d -> c b d') #for transformer
        ...
        output = rearrange(output, 'c b d -> b c d')
        output_bbox = self.output_process(output) 
        output_geometry = torch.cat((output_bbox, sample["geometry"][:, :, 4:6]), dim = -1)
        
        return output_geometry # x,y, w,h, cos r, sir r, z
